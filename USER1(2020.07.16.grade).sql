...
-- 지금 1개의 값만 보일지라도 이 결과는 리트스(다수) 형으로 출력된다.
SELECT * FROM tbl_student
WHERE st_name= '남동예';

-- PK칼럼에 2개이상으로 조건을 부여하여 조회하기
-- 만약 결과가 1개의 Record만 보일지라도 결과는 리스트 형으로 출력된다(취급한다)
SELECT * FROM tbl_student
WHERE st_num = '20010' OR st_num = '20020' OR st_num = '20030';

-- 1개의 칼럼에 다수의 OR조건을 부여하여 SELECTION을 할때는
-- IN 연산자를 사용할 수 있다.
SELECT * FROM tbl_student
WHERE st_num IN ('20010','20020','20030');

-- 1개의 칼럼 값을 범위로 제한하는 SELECTION 할때
...
-- 범위를 저장할떄 시작값과 종료같이 포함되는 연산을 수행할때는
-- BETWEEN 연산자를 사용할 수 있다.
SELECT * FROM tbl_student
WHERE st_num BETWEEN '20010' AND st_num '20020' ;

------------------------------------------------
----------- PROJECTION
-----------table에 있는 칼럼중에서 내가 보고자 하는 칼럼만 나타나도록 제한하느것
...
SELECT sc_num , sc_kor, sc_eng, sc_math, sc_music, sc_art,
        ...
        --ROUND(값,자릿수) : 자릿수 +1 에서 반올림하여 자릿수자리까지 소수점 표현
        ...
        TRUNC(sc_kor + sc_eng + sc_math + sc_music + sc_art)/ 5,2 AS 평균     
FROM tbl_score ;
...
SELECT SUBSTR(sc_num) AS 반,
     sum(sc_kor) AS 국어총점,
     sum(sc_eng) AS 영어총점
     
FROM tbl_score -- where 는 from 다음에 나타나야 한다.
WHERE 1 = 1 OR sc_num = '20001'
GROUP BY SUBSTR (sc_num,1,4)
ORDER BY 반; -- ORDER BY 는 SELECT 명령문의 절들 중에 가장 끝네 나타 나야한다
