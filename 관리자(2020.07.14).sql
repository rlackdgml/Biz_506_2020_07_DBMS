...
-- 그중에서 tab 이라는 이름의 table정보를 가져와서 나에게 보여달라
--  오라클의 tab table은 현재 접속된 사용자가 관리하는 
-- DB Object(객체)들의 정보를 보관하는 있는 table
SELECT *
FROM Tab;

-- ALL_ALL_TABLES 오라클 system 데이터 사건의 자세한 정보를 보관하는 table
SELECT *
FROM ALL_ALL_TABLES;

-- SELECT 키워드 FROM 절을 포함하는 명령문 형태로 작성을하며
-- DBMS 보관하고 있는 데이터를 TABLE 형식으로 보여달라 라는 명령
-- DBMS DML(Database Manuplation Lang)중에서
-- CRUD 중에서 R : READ, Retrive 를 수행하는 명령문
SELECT *
FROM 주소록;

-- sql 명령문을 통해서 DB 객체를 만들고 삭제하고, 데이터를 추가, 변경 삭제를 수행할텐데
-- SYS 사용자로 접속을 하게 되면 중요한 정보를 잘못 삭제 변경할 우리가 있기 때문에
-- 실습을 위해서 사용자를 생성하여 수행을 할 것이다.

-- 사용자를 추가하는 순서
-- 1. Table Space : 데이터를 저장할 물리적 공간을 
...
-- '..../myYS.dbf' : 저장할 파일 이름
-- Size : 오라클에서는 성능의 효율성을 주기위해 일단 빈 공간을 일정부분 설장하게 된다
--        크기는 최초에 저장할 데이터의 크기등을 계산하여 설계하고 설정한다.
--        너무 작으면, 효율성이 떨어지고, 너무 크면 불필요한 공간을 낭비한다.
--        오라클 xe(Express Edition)에서는 Table Space의 최대 크기를 11G를 제한한다
--        만약 Size 10G로 지정하고 용량이 초과되어 AUTO NEXT로 추가가 되는경우
--        전체 Size 11G를 넘어서면 오류가 나면서 더이상 데이터를 저장수 없게 된다

--AUTO..NEXT : 만약 초기에 지정한 Size공간에 데이터가 모두 가득차면 
--             자동으로 용량을 늘려서 저장할수 있도록 만들어라

-- Size의 1M : 기본크기를 1024 * 1024 byte 크기로 저장하라. Size를 지정할떄 1MB 라고 하지 않는다.
-- Next 500K : 자동으로 확장(늘리기)를 1024 * 500 크기로 설정
--              Next를 지정할때 500K 라고 하지 않는다.    

-- CREATE 시작되는 명령문 : DDL(Data definiton Lang) : 데이터 선언, 생성(추가와는 다름)
...
-- DROP 명령은 데이터를 물리적으로 완적삭제하는 개념이므로 매우 신중하게 사용해야 한다.
...
-- user1이 로그인만 할수 있도록 권한(역할)을 부여하라
GRANT CONNECT TO user1;
-- user1이 로그인할수 있는 권한을 제거
REVOKE CONNECT FROM user1;

-- user1 이 로그인을 수행하고, 최소한으로 데이터들을 관리할 수 있도록 권한을 부여
...
-- CONNECT 와 RESOURCE 권한을 부여하게 되면 거의 DBA 수준의 권한을 갖게된다.
...
GRANT CONNECT,CREATE TABLE, INSERT TABLE 학생정보 SELECT 학생정보 TO user1;
...
-- DBA 권한(Roll)은 SyDBA보다 한단계 낮은 권한 등급을 가지며
