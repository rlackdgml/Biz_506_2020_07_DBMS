...
-- Record의 제한(전체 데이터개수 > 조건에 맞는데이터 개수)
...
-- st num 에 저장된 값이 '20001' 이상이고 and '200010'
-- 데이터리스트(record) 만 보여달라
-- PK, UNIQUE 로 설정된 칼럼을 복합 조건절로 SELECTION할때는
--     레코드가 1개만 나타날 지라도 이 데이터는 리스트로 취급한다.
SELECT * FROM tbl_student
WHERE st_num >= '20001' AND st_num <='20010';

-- st_name에 저장된 값이 나한율 인 리스트만 보여라
-- PK, UNIQUE 로 설절된 칼럼이 아닌경우
-- SELECTION 을 수행했을때 나타는 레코드가 1개 일지라도
--         이 데이터는 리스트로 취급한다.
...
SELECT st_num, st_name,st_dept,st_grade
FROM tbl_student
WHERE st_num > = '20001'AND st_nym = '20010';
-- '20001' <= st_num >= '20010' 이랗게 사용하면 안됨
...
-- 한계의 칼럼에 다수의 OR 조건을 부여하여
-- SELECTIOM을 수행할때는 IM 연산자를 사용하여 구현할수 있다.
