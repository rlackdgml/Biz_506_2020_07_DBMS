...
-- 데이터를 INSERT 수행할떄 DBMS 차원에서 유효성검사를 하고
...
-- UNIQUE : 중복되지 않는 값만 유호, 해당칼럼에 추가하고자하는 데이터가
...
--       저장하는 Data 가 설정한 자릿수보다 작으면 칼럼의 실제 크기를 줄여서 저장
...
--       영문자 전용이 아닌 한글, 한자 등 알파벳 이외의 문자를 지정할때 사용
--       UNICODE 문자열을 위해 특별히 마련된 Type
--       영문자1글자와 한글1글자륾 같은 자릿수로 취급한다.
...
-- ORA-00001: unique constraint (GRADE.SYS_C007012) violated : PK나 , UNQUE로
--        설정된 칼럼에 이미 저장된 값을 또 저장하려고 할때
...
-- INSERT 수행후 자주 발생하는 오류들
-- invalid number :
--     NUMBER로 성정된 칼럼에 문자열을 저장하려고 시도한 경우
--     만약 NUMBER 설정된 칼럼에 숫자를 '3' 따옴표로 묶어서 지정하면
...
-- INSERT 수행후 자주 발생하는 오류들
--  check constraint (GRADE.SYS_C007011) violated :
--   CHECK 로 유효성 검사를 설정했는데 범위, Valid를 통과하지 못했을때
