...
-- Table 생성, CRUD 등 명령을 사용할수 있다.

-- DB에 데이터를 저장하기
-- DB를 대상으로 업무를 수행할때 데이터가 있어야만 여러가지 잃을 수행할수 있다.
-- CRUD 중에서 가장 먼저 수행해야할 명령이 Create 형 명령
-- DM CRUD Create DDL CRATE 명령과 구분을 해야한다
-- DDL의 CREATE : 생성이라는 개념이고 운영체와 밀접한 관련이 있거나, 
--   물리적인 요소가 많다.
--   CREATE TABLSPACE, CREATE USER 와 같이 저장공간, 사용자 정도 이며 Schema들을
--   생성하는 명령 절차

-- DML(Data Manuplation Lan, (Data Manuplation Lang  )에서 Create
--   물리적 저장공간에 실제 발생된 Data를 추가 저장하기
--   아직 저장되지 않은 논리적 개념의 데이터를 로컬스토리에 보관하는 개념
--   DML의 Create 형 명령 : "INSERT"

-- RDBMS(Relationship Database Manuplation System, 관계형데이터베이스 시스텀 )에서
-- 데이터를 추가하려면, 먼저 데이터 저장공간에 대한 정의를 수행해야 한다.
-- 데이터 저장공간을 논리적으로 Entity, 물리적으로 TABLE 이라고 한다.

-- TABLE : 표준 SQL(ANSI SQL)에서 데이터를 추가하는 저장공간
--       저장할 데이터의 각 필드 항목의 type 결정
--       (학생정보 : 학번, 이름, 학과, 학년, 전화번호, 주소, 나이) 후보키
...
-- Table 명명규칙 : 보통 tbl_ 접두사로 시작한다. snack case 형식으로 이름 저장
CREATE TABLE tbl_student (
    -- 칼럼(Fleld) 명명규칙 : 보통 table이름을 줄여서 접두사 시작
    --   snack case 형식으로 이름 지정
    -- 문자열 칼럼의 Type
    -- CHAR, VARCHAR2
    -- CHAR : 고정길이 문자열, 칼럼에 저장되는 데이터의 길이가 모두 같은 경우에
    --       주로 사용된다.
    --       저장되는 데이턱 설정한 크기보다 작으면 남는 공간을 공백으로 채운다.
...
-- 데이터를 INSERT 할때 혹시 실수로 PK 칼럽에 이미 저장된 데이터를
...
-- 두개의 칼럼을 나열했는데 VALUES의 3개의 데이터를 지정하면 INSERT 오류 발생
...
-- 어떤 이유로 SELECT 수행했을 때 성춘향 데이터가 1개빡에 보이지 않았더라도
...
-- 장보고 학생의 데이터가 필요가 없어져 삭제를 하려고한다.
...
-- 데이터를 Table형식으로 보여줄떄 모든 칼럼을 다 포함하여 보여달라
-- SELECT TYPE 으로 TABLE 보기
SELECT * FROM tbl_student;

-- 데이터를 Table 형식으로 보여줄떄 나열된 칼럼들만 보여달라
...
-- 문자열 데이터를 저장하는 칼럼
-- CHAR : 데이터의 길이가 고정된 칼럼, 코드(검색하는 용도의 칼럼)등 저장하는 칼럼
...
-- 학과가 컴퓨터 공학 이고(and) 3학년인 학생들만 보여달라
SELECT *
FROM tbl_student
...
SELECT st_name , st_dept
FROM tbl_student 
WHERE st_dept IN ('컴퓨터공학' OR st_dept = '법학');
...
-- 문자열 칼럼의 저장된 데이터가 모두 길이가 같고 format이 값은경우
-- 부등호(범위연산)을 포함하여 값을 조회할수 있다.
...
-- 시작값과 종요값이 포함된 범위를 조회하는 코드는
-- BETWEEN 연션자를 사용하여 조회하는 코드로 변환하여 사용할 수 있다.
SELECT st_num, st_name, st_dept,st_grade
FROM tbl_student
