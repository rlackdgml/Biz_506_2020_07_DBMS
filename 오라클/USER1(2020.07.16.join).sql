...
SELECT  st_num, st_name, st_dept, d_name,d_porf, d_tel , st_grade, st_tel, st_addr 
FROM tbl_student
    LEFT JOIN tbl_dept
        ON st_dept = d_code;
...
-- 작성된 Query를 자주 사용하게 될것으로 에상이 되면
-- 이 Query를 View 생성해 보관할 수 있다.

-- View 는 실제 Table과 똑같이 SELECT 명령을 중복 데이터를 조회할 수 있다.
...
(
    ...
    SELECT SC.sc_num,ST.st_name,ST.st_tel,
        ST.st_dept,DT.d_name,DT.d_porf,DT.d_tel,
        SC.sc_kor,
        SC.sc_eng,
        SC.sc_math,
        SC.sc_music,
...
AS
(
SELECT st_num,st_name,st_dept,DT.d_name
FROM tbl_student ST
    LEFT JOIN tbl_dept DT
        ON ST.st_dept = DT.d_code
);
