...
-- 외래키(FOREIGN Key)
-------------------------------
-- 두개 (이상) 의 테이블간에 Realtion을 강제로 설정하여
-- 테이블에 추가되는 데이터의 무결성을 보중하는 제약조건 설정
-- JOIN 사용하여 두개의 테이블에 Realtion을 설정하기도 하는데
-- 그 차이점
-- JOIN은 단순히 View 의 편리함(필요한 정보를 Projection 포함)을 위해서 사용하기
-- FK 는 데이터를 추가하는 단계부터 잘못된 데이터가 추가되는 것을 방지하는 목적
-- FK 는 테이블을 생성할떄 만들기도 하지만
-- 일반적으로 테이블을 생성한 후 테이블 변경을 통해 만드는 것이 좋다.

-- 가장 좋은 경우는 테이블을 생성하고 데이터가 한개도 추가하지 않은 상태로
-- 설정하는 것이다
...
-- 현재 스키마에 있는 학생정보와 성적정보 테이블을 FK(외래키) 관계로 성정을 한다.
-- FK 관계의 테이블을 부모테이블과 자식테이블로 분류하는데, 
-- FK 설정은 자식테이블에서 실시한다.

-- 학생정보 테이블에 학생정보가 없으면, 성적정보를 입력할 수없도록 설정한다.
-- 학생정보 테이블을 부모테이블이라고 하고, 성적정보 테이블을 자식 테이블이라고 한다.
SELECT * FROM tbl_score;

-- 사후(테이블 생성후) FK 설정
ALTER TABLE tbl_score        -- tbl_score(자식) 테이블에 설정을 변경하겠다. (자식테이블에서 만든다)
...
--      1. 먼저 성적데이터에서 변경할 학번의 성적데이터를 모두 삭제하고
--      2. 학생정보의 학번을 변경하고
...
ADD CONSTRAINT FK_D_ST -- 한 스키마내에서 이름을 유일해야 한다.
FOREIGN KEY (st_dept)
