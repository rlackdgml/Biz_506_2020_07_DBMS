...
--      이러한 문제를 방지하기 위해 다음과 같은 정류화를 실행한다.
...
-- 5. 그결과를 현재 iolist의 레코드의 io_pcode 칼럼에 UPDATE를 수행한다
...
-- b_tel 칼럼의 값이 중복된(2개이상인) 레코드가 있냐
...
-- iolist 와 buyer 테이블간의 거래처명, 대표자명 칼럼으로 JOIN을 수행하여 데이터 검증
